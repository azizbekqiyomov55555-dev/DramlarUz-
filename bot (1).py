...
# ─── KONFIGURATSIYA ────────────────────────────────────────
BOT_TOKEN  = os.environ.get("BOT_TOKEN")  or "8723400610:AAGqjQUbm-o_YVx5-J4_iQTMOMjggMdJ3RY"
...
def db_initial_load():
    """
    Ishga tushganda:
    1. JSONBlob dan yukla (asosiy)
    2. Agar JSONBlob ishlamasa — lokal fayldan yukla
    3. Hamma narsani RAMga yoz
    """
    logger.info("🔄 Ma'lumotlar yuklanmoqda...")

    # 1. JSONBlob
    blob = _load_jsonblob()
    if blob and isinstance(blob, dict) and (blob.get("movies") or blob.get("users")):
        RAM.from_dict(blob)
        EMOJI_IDS.clear()
        EMOJI_IDS.update(RAM.emoji_ids)
        _save_local(blob)  # lokal backup yangilash
        logger.info(f"✅ JSONBlob → RAM: {len(RAM.movies)} kino, {len(RAM.users)} user")
        return

    # 2. Lokal fayl
    local = _load_local()
    if local and isinstance(local, dict) and (local.get("movies") or local.get("users")):
        RAM.from_dict(local)
        EMOJI_IDS.clear()
        EMOJI_IDS.update(RAM.emoji_ids)
        logger.info(f"✅ Lokal fayl → RAM: {len(RAM.movies)} kino, {len(RAM.users)} user")
        # Fon rejimida JSONBlob ga yoz
...
# ══════════════════════════════════════════════════════════
# SAQLASH — RAM → lokal + JSONBlob (async)
# ══════════════════════════════════════════════════════════

# Debounce: ko'p o'zgarish bo'lsa bitta saqlash
_pending_save_event = asyncio.Event() if False else None  # _setup da yaratiladi

async def schedule_save():
    """
    RAMni saqlash navbatga qo'shadi.
    Bir nechta tez-tez chaqirilsa, faqat 1 ta saqlash bo'ladi (debounce 2 sek).
    """
    DB_STATUS["pending_save"] = True
    # Lokal faylga darhol yoz (tez, ishonchli)
    data = RAM.to_dict()
    _save_local(data)
    # JSONBlob ga fon rejimida yoz
    asyncio.create_task(_bg_save_jsonblob())


_save_in_progress = False

async def _bg_save_jsonblob():
    """JSONBlob ga fon rejimida saqlash"""
    global _save_in_progress
    if _save_in_progress:
        return  # allaqachon saqlanmoqda
...
async def save_now() -> bool:
    """Darhol saqlash (await qilish mumkin)"""
    data = RAM.to_dict()
    _save_local(data)
    ok = await asyncio.to_thread(_save_jsonblob, data)
    now_str = datetime.now().strftime("%H:%M:%S")
...
def find_movie_code(query: str):
    ...
    for c, movie in movies.items():
        ...
        if q == title_norm or q == code_norm:
            exact.append(c)
        elif q in title_norm or title_norm in q or q in code_norm:
            partial.append(c)
...
def _build_ep_price_list(code: str, eps: list, prices: dict) -> str:
    ...
    for i in range(len(eps)):
        ek = str(i + 1)
        price = prices.get(ek)
        if price: lines.append(f"  {ek}-qism — 💰 <b>{price} so'm</b>")
...
def generate_movies_image(movie_slice: list, page: int = 1, total_pages: int = 1,
    ...
    SCALE    = 3   # 8K sifat uchun (820*3 ≈ 2460px kenglik, super tiniq)
    IMG_W    = 820  * SCALE
    PAD_X    = 22   * SCALE
    TOP_PAD  = 14   * SCALE
    CARD_H   = 92   * SCALE
    ...
    fnt_header = try_font(34 * SCALE)
    fnt_num    = try_font(22 * SCALE)
    fnt_title  = try_font(20 * SCALE)
    fnt_sub    = try_font(14 * SCALE)
    fnt_footer = try_font(17 * SCALE)
    ...
    for idx, (code, movie) in enumerate(movie_slice):
        ...
        draw.rounded_rectangle([x0, y0, x1, y1], radius=12*SCALE, fill=WHITE, outline=col, width=3*SCALE)
        draw.rounded_rectangle([x0, y0, x0 + 7*SCALE, y1], radius=4*SCALE, fill=col)
        bx0 = x0 + 16*SCALE
        bx1 = bx0 + BADGE_SZ
...
async def cb_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "bc_cancel":
        for k in ["bc_msg", "bc_buttons", "bc_adding_btn", "bc_btn_name"]:
...
async def cb_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pay["status"] = "approved"
    uid = str(pay["user_id"])
    RAM.ensure_user(uid)["paid_episodes"][f"{pay['code']}_{pay['ep']}"] = True
    try:
        next_ep = str(int(pay["ep"]) + 1)
        RAM.users[uid]["paid_episodes"][f"{pay['code']}_{next_ep}"] = True
    except: pass
    await schedule_save()
    try:
        await q.edit_message_caption(
...
async def admin_state_handler(update, context, text: str) -> bool:
    ...
    if state == "set_kino_kanal":
        ...
        if text.strip() == "0":
            ...
            await save_now()
            await sm(context.bot, uid, "✅ Kino kodlari kanali linki <b>o'chirildi</b>!", admin_menu_kb())
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid == ADMIN_ID and state == "add_ep_video":
        ...
        # ❗ Darhol RAMga yoz
        RAM.movies[code]["episodes"].append(msg.video.file_id)
        ep_num = len(RAM.movies[code]["episodes"])

        # Bazaga saqlash (retry bilan)
        save_ok = await save_now()
        if not save_ok:
        ...
        movie = RAM.movies[code]
        storage_warn = "\n⚠️ <i>Lokal saqlandik, JSONBlob xato!</i>" if not save_ok else ""
        await sm(context.bot, uid,
            f"✅ <b>{ep_num}-qism</b> RAMga saqlandi!\n"
            f"Kino: <b>{movie.get('title', code)}</b>\n"
            f"Kod: <code>{code}</code>\n"
            f"Jami qismlar: <b>{ep_num} ta</b>{storage_warn}\n\n"
