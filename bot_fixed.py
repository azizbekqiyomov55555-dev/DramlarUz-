...
def db_load():
    ...
    if os.path.exists(LOCAL_BACKUP_FILE):
        try:
            ...
            EMOJI_IDS.clear()
            EMOJI_IDS.update(data.get("emoji_ids", {}))
            logger.warning("⚠️ JSONBlob ishlamadi — LOKAL BACKUPDAN yuklandi!")
...
async def db_save_async(data):
    """Async saqlash — JSONBlob + lokal backup."""
    if not _load_ok:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.put(
                    ...
                    if resp.status in (200, 201):
                        logger.info("DB saqlandi ✓ (JSONBlob + backup)")
                        return True
                    else:
                        txt = await resp.text()
    ...
    logger.warning("⚠️ JSONBlob'ga saqlanmadi, ammo lokal backup mavjud")
    return False


def db_save(data):
    """Sinxron saqlash — JSONBlob + lokal backup."""
    if not _load_ok:
    ...
    with _save_lock:
        ...
        for attempt in range(3):
            try:
                r = requests.put(JSONBLOB_URL,
                    headers=JSONBLOB_HEADERS,
                    data=payload.encode("utf-8"), timeout=30)
                if r.status_code in (200, 201):
                    logger.info("DB saqlandi ✓ (sync JSONBlob)")
                    return True
                else:
