...
if not BOT_TOKEN:
    raise RuntimeError(
        ...
        "BOT_TOKEN=... python DramlarUzn_tuzatilgan.py"
    )
...
def ibtn(text, data=None, url=None, style=None, emoji_id=None):
    """
    Telegram Bot API inline tugmasi.
    style va emoji_id argumentlari eski kod bilan moslik uchun qoldirilgan,
    lekin Telegram Bot API ularni qabul qilmaydi — yuborilsa tugma chiqmay qoladi.
    """
    b = {"text": text}
    if data is not None:
...
def rbtn(text, style=None, emoji_id=None):
    """
    Oddiy keyboard tugmasi. style/emoji_id Telegram reply keyboardda ishlamaydi.
    """
    return {"text": text}
...
PHOTO_PAGE_SIZE = 10   # Har bir suratda nechta kino ko'rsatilsin
...
async def _send_kino_list_page(bot, chat_id: int, page: int = 0, edit_query=None):
    """
    "Barcha kinolar" uchun 10 tadan sahifalangan katta rasm yuboradi.
    1-sahifada rasm tagida "Qolgan kinolar" tugmasi chiqadi.
    Callback bosilganda shu xabar ichidagi rasm yangilanadi.
    """
    movies = DB.get("movies", {})
    if not movies:
        return

    # Eng yangi kinolar birinchi chiqadi
    ...
    # ── Botning o'zi katta rasm yasaydi ───────────────────
    img_buf = None
    if PIL_AVAILABLE:
        try:
            ...
            if img_buf:
                img_buf.name = f"kinolar_{page + 1}.jpg"
        except Exception as e:
            logger.error(f"kino_list surat xato: {e}")

    # ── Rasm tagidagi tugmalar ────────────────────────────
    nav_row = []
    if page > 0:
        nav_row.append(ibtn("⬅️ Oldingi", data=f"kino_list|{page - 1}", style="primary"))
    if page < total_pages - 1:
        next_text = "📂 Qolgan kinolar" if page == 0 else "Keyingi ➡️"
        nav_row.append(ibtn(next_text, data=f"kino_list|{page + 1}", style="primary"))
    ...
    if img_buf:
        # Inline tugma bosilganda yangi xabar yubormay, o'sha rasmni almashtiramiz.
        if edit_query is not None:
            try:
                ...
                logger.warning(f"kino_list edit xato, yangi rasm yuboriladi: {e}")
                try:
                    img_buf.seek(0)
                except Exception:
                    pass
    ...
    # Agar Pillow o'rnatilmagan bo'lsa, vaqtincha matn ro'yxat yuboriladi.
    lines = [
        f"{start + i + 1}. 🎬 <b>{_strip_html(m.get('title', c))}</b>  "
        f"📌 <code>{c}</code>  👁 {sum(m.get('views', {}).values())}"
        for i, (c, m) in enumerate(slice_items)
    ]
    text = f"🎬 <b>Kinolar</b> ({start + 1}–{end} / {total_count}):\n\n" + "\n".join(lines)
    if edit_query is not None:
        try:
            await edit_query.edit_message_text(text=text, parse_mode="HTML", reply_markup=kb)
            return
        except Exception as e:
            logger.warning(f"kino_list text edit xato, yangi xabar yuboriladi: {e}")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("kino_list|"):
        try:
            ...
            pg = 0
        await _send_kino_list_page(context.bot, uid, page=pg, edit_query=q)
        return
