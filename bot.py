# -*- coding: utf-8 -*-
"""
Kino Bot - Bot API 9.4 rangli tugmalar
Emoji: admin xohlagan emoji yuboradi — tugma textiga qo'shiladi
"""
...
# Tugma textlari — admin emoji qo'shsa "emoji matn" ko'rinishida saqlanadi
DEFAULT_BTN = {
    # Reply tugmalar
    "yordam":       "Yordam",
...
def bt(key):
    """Tugma textini qaytaradi — admin o'zgartirgani yoki default"""
...
def ibtn(text, data=None, url=None, style=None):
    b = {"text": text}
...
def main_menu_kb():
    return rkb([[
        rbtn(bt("yordam"),  style="primary"),
        rbtn(bt("install"), style="success"),
    ]])

def admin_menu_kb():
    return rkb([
        [rbtn(bt("kino_joy"),  style="success"),
         rbtn(bt("qism_qosh"), style="primary")],
        [rbtn(bt("pullik"),    style="danger"),
         rbtn(bt("stat"),      style="primary")],
        [rbtn(bt("kanal_post"),style="primary"),
         rbtn(bt("maj_kanal"), style="danger")],
        [rbtn(bt("karta"),     style="success"),
         rbtn(bt("ilova"),     style="primary")],
        [rbtn(bt("emoji_soz"), style="primary")],
        [rbtn(bt("asosiy"),    style="success")],
    ])

def subscription_kb(channels):
    rows = [[ibtn(c['title'], url=c["url"], style="primary")] for c in channels]
    rows.append([ibtn(bt("tekshir"), data="check_sub", style="success")])
    return ikb(rows)
...
def payment_admin_kb(pid):
    return ikb([[
        ibtn(bt("tasdiq"), data=f"pay_ok|{pid}", style="success"),
        ibtn(bt("bekor"),  data=f"pay_no|{pid}", style="danger"),
    ]])

def share_kb(url):
    return ikb([[ibtn(bt("ulash"), url=url, style="primary")]])
...
def reply_admin_kb(uid):
    return ikb([[ibtn(bt("javob"), data=f"reply|{uid}", style="primary")]])

def stats_kb():
    return ikb([[ibtn(bt("yangi"), data="refresh_stats", style="primary")]])

def movie_added_kb(code):
    return ikb([[
        ibtn(bt("qism_add"), data=f"quick_add_ep|{code}", style="success"),
        ibtn(bt("narx_bel"), data=f"quick_price|{code}",  style="primary"),
    ]])

def payment_sent_kb():
    return ikb([[ibtn(bt("kut"), data="waiting_confirm", style="primary")]])

def help_kb():
    return ikb([[ibtn(bt("bosh"), data="go_home", style="success")]])

def emoji_list_kb():
    """Tugma emoji sozlamalari paneli"""
    rows=[]; keys=list(BTN_LABELS.keys())
    for i in range(0,len(keys),2):
        row=[]
        for key in keys[i:i+2]:
            cur = DB.get("btn_texts",{}).get(key) or DEFAULT_BTN.get(key,"")
            row.append(ibtn(f"{BTN_LABELS.get(key,key)}", data=f"emoji_edit|{key}", style="primary"))
        rows.append(row)
    rows.append([ibtn(bt("tiklash"), data="emoji_reset_all", style="danger")])
    rows.append([ibtn(bt("yopish"),  data="emoji_close",     style="success")])
    return ikb(rows)
...
async def cb_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="emoji_close":
        await q.edit_message_text("Sozlamalar yopildi."); return
    if data=="emoji_reset_all":
        DB["btn_texts"]={};save()
        await q.edit_message_text("<b>Barcha tugmalar tiklandi!</b>",parse_mode="HTML",reply_markup=emoji_list_kb()); return
    if data.startswith("emoji_reset|"):
        key=data.split("|")[1]; DB["btn_texts"].pop(key,None); save()
        kb,cur,default=emoji_single_kb(key)
        await q.edit_message_text(
            f"<b>{BTN_LABELS.get(key,key)}</b> tiklandi!\n\nDefault matn: <code>{default}</code>",
    ...
    if data.startswith("emoji_edit|"):
        ...
        await q.edit_message_text(
            ...
            f"Hozirgi ko'rinish: <code>{cur}</code>\n\n"
            f"Yangi emoji + matn yuboring\n"
            f"<i>Masalan: 🎬 Kino joylash</i>\n"
            f"<i>Yoki faqat emoji: 🎬</i>",
            parse_mode="HTML",reply_markup=kb); return
...
async def cb_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pay: await q.edit_message_caption("To'lov topilmadi."); return
    if action=="pay_no":
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Admin tugma matni o'zgartirish rejimi
    if uid==ADMIN_ID and context.user_data.get("editing_btn_key"):
        ...
        if not text.strip():
            ...
            context.user_data["editing_btn_key"]=key; return
        DB.setdefault("btn_texts",{})[key]=text.strip(); save()
        await sm(context.bot,uid,
            f"<b>{BTN_LABELS.get(key,key)}</b> tugmasi yangilandi!\n\n"
            f"Yangi ko'rinish: <code>{text.strip()}</code>\n\n"
            f"<i>Bot restart qilinsa o'zgarishlar ko'rinadi.</i>",
            emoji_list_kb())
        return

    # Admin tugmalarini aniqlash (bt() orqali — chunki textlar o'zgarishi mumkin)
    all_admin_btns = {bt(k) for k in ["kino_joy","qism_qosh","pullik","stat",
        "kanal_post","maj_kanal","karta","ilova","emoji_soz","asosiy"]}
    if uid==ADMIN_ID and text in all_admin_btns:
        if text==bt("emoji_soz"):
            await sm(context.bot,uid,
                "<b>Tugma sozlamalari</b>\n\nO'zgartirmoqchi bo'lgan tugmani tanlang:\n"
                "<i>Emoji + matn yuboring, masalan: 🎬 Kino joylash</i>",
                emoji_list_kb()); return
        await admin_buttons(update,context,text); return

    if text==bt("yordam"):
        await sm(context.bot,uid,"Savol yoki muammoingizni matn, rasm yoki video ko'rinishida yuboring.\nAdmin tez orada javob beradi.",help_kb())
...
async def admin_buttons(update, context, text):
    ...
    if text==bt("asosiy"): await sm(context.bot,uid,"Asosiy menyu",main_menu_kb()); return
    if text==bt("stat"):
        u=len(DB.get("users",{})); m=len(DB.get("movies",{})); v=DB.get("stats",{}).get("total_views",0)
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid==ADMIN_ID and "reply_to" in context.user_data:
        target=context.user_data.pop("reply_to")
        try:
            if msg.photo: await sp(context.bot,target,msg.photo[-1].file_id,"<b>Admin javobi</b>")
            elif msg.video: await sv(context.bot,target,msg.video.file_id,"<b>Admin javobi</b>")
            await sm(context.bot,uid,"Yuborildi!")
        except Exception as e: await sm(context.bot,uid,f"Xato: {e}")
