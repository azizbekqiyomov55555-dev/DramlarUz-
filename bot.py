...
# ─── KONFIGURATSIYA ────────────────────────────────────────
BOT_TOKEN  = os.environ.get("BOT_TOKEN")  or "8723400610:AAHtJWYXYw0ObIpkiy34Do5x4f1TzmRbSeI"
...
TOP_N = 10  # Birinchi suratdagi top yangi kinolar soni


async def _send_kino_list_page(bot, chat_id: int, page: int = 0):
    """
    page=0 → TOP 10 eng yangi kino (rasm) + "📂 Qolgan kinolar ro'yxati" tugmasi
    page=1 → QOLGAN barcha kinolar BITTA uzun rasmda (tepadan pastga sirpantirib ko'rish uchun)
    """
    movies = DB.get("movies", {})
    if not movies:
        return

    # Eng yangi — oxiridan boshlaymiz (yangi qo'shilganlar birinchi)
    all_items   = list(movies.items())[::-1]
    total_count = len(all_items)

    if page == 0:
        slice_items  = all_items[:TOP_N]
        start_offset = 0
        header_caption = (
            f"🔥 <b>TOP {len(slice_items)} — Yangi qo'shilgan kinolar</b>\n"
            f"📋 Jami: <b>{total_count} ta</b>\n\n"
            f"Kino <b>kodini</b> yuboring — video <b>darhol</b> keladi! ⚡"
        )
    else:
        slice_items  = all_items[TOP_N:]   # qolgan hammasi bitta rasmda
        start_offset = TOP_N
        if not slice_items:
            await bot.send_message(chat_id=chat_id,
                text="📭 <b>Qolgan kinolar yo'q.</b>", parse_mode="HTML")
            return
        header_caption = (
            f"📂 <b>Qolgan kinolar ro'yxati</b>\n"
            f"📋 Ko'rsatilmoqda: <b>{TOP_N + 1}–{total_count}</b>  |  Jami: <b>{total_count} ta</b>\n\n"
            f"⬇️ Tepadan pastga sirpantirib ko'ring.\n"
            f"Kino <b>kodini</b> yuboring — video <b>darhol</b> keladi! ⚡"
        )
    ...
    # ── Inline tugmalar ───────────────────────────────────
    rows = []
    if page == 0 and total_count > TOP_N:
        rows.append([ibtn("📂 Qolgan kinolar ro'yxati",
                          data="kino_list|1", style="primary")])
    ...
    if img_buf:
        # Telegram rasm balandligi cheklangan (~10000px, nisbat 20:1).
        # Uzun bo'lsa hujjat sifatida yuboramiz — sifat yo'qotmasdan
        # foydalanuvchi to'liq tepadan pastga sirpantirib ko'ra oladi.
        try:
            from PIL import Image as _PILImage
            img_buf.seek(0)
            with _PILImage.open(img_buf) as _im:
                w, h = _im.size
            img_buf.seek(0)
            send_as_doc = (h > 8000) or (h / max(w, 1) > 18)
        except Exception:
            send_as_doc = False

        if send_as_doc:
            img_buf.name = "kinolar.jpg"
            await bot.send_document(
                chat_id=chat_id, document=img_buf,
                caption=header_caption, parse_mode="HTML",
                reply_markup=kb,
            )
        else:
        ...
        # Matn fallback
        lines = [
            f"{start_offset+i+1}. 🎬 <b>{_strip_html(m.get('title', c))}</b>  "
