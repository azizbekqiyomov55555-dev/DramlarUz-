    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code == 200:
                ...
                # Har bir bo'lim bor-yo'qligini va to'g'ri turini ta'minlash
                for k, default_v in DEFAULT_DB.items():
                    if k not in data:
                        data[k] = json.loads(json.dumps(default_v))
                    elif isinstance(default_v, dict) and not isinstance(data[k], dict):
                        logger.warning(f"⚠️ '{k}' noto'g'ri tur, dict ga tuzatildi")
                        data[k] = json.loads(json.dumps(default_v))
                    elif isinstance(default_v, list) and not isinstance(data[k], list):
                        logger.warning(f"⚠️ '{k}' noto'g'ri tur, list ga tuzatildi")
                        data[k] = json.loads(json.dumps(default_v))
...
def main():
    ...
    while True:
        try:
            ...
            logger.info("Polling normal tugadi.")
            break
            ...
            if "Conflict" in msg or "terminated by other getUpdates" in msg:
                logger.warning("⚠️ Boshqa instance ishlamoqda. 30s kutamiz va qayta urinamiz...")
                try:
                ...
                time.sleep(30)
                continue
            logger.error(f"❌ Xato: {e}. 10s dan keyin qayta...")
            time.sleep(10)
            continue

    logger.info("💾 Saqlanyapti...")
    try:
        save()
    except Exception as e:
