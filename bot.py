...
def _load_from_jsonbin():
    ...
    Bot start'da TO'LIQ yuklash uchun kuchli retry."""
...
async def db_save_async(data):
    """Async saqlash: lokal backup + JSONBin + JSONBlob (zaxira).
    TUZATISH (v11): _load_ok bloklash olib tashlandi — har doim saqlaydi.
    Lokal har doim yoziladi; JSONBin va JSONBlob parallel yoziladi."""
    data["emoji_ids"] = dict(EMOJI_IDS)
    payload = json.dumps(data, ensure_ascii=False)

    # 1) Lokal backup darhol — agar internet o'chsa ham ma'lumot saqlanadi
    _save_local(payload)

    # 2) JSONBin + JSONBlob ga parallel yozish
    ok_bin = False
    ok_blob = False
    try:
...
def main():
    ...
    # Har 3 daqiqada JSONBin'dan tekshiradi. Agar serverda KO'PROQ kino
    # bo'lsa (boshqa instance qo'shgan), xotirani yangilaydi.
    # HECH QACHON kamaytirmaydi — ma'lumot yo'qolishidan himoya.
    async def _periodic_refresh(context):
        ...
        try:
            ...
            # Faqat YANGI kinolar qo'shilgan bo'lsa yangilaymiz
            if new_movies > cur_movies:
                # Mavjud kinolarni saqlab, yangilarini qo'shamiz (merge)
                merged_movies = dict(fresh.get("movies", {}))
                merged_movies.update(DB.get("movies", {}))  # lokal o'zgarishlar ustun
                # Lekin server'da yangi kinolar bo'lsa, ularni ham qo'shamiz
                for code, mv in fresh.get("movies", {}).items():
                    if code not in merged_movies:
                        merged_movies[code] = mv
                DB["movies"] = merged_movies
                # Userlarni ham merge qilamiz
                for uid, u in fresh.get("users", {}).items():
                    if uid not in DB.get("users", {}):
                        DB.setdefault("users", {})[uid] = u
                logger.info(f"🔄 JSONBin refresh: {cur_movies} → {len(DB['movies'])} kino")
        except Exception as e:
