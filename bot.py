...
async def db_save_async(data):
    ...
    async with _async_save_lock:
        ...
        if ok_bin and ok_blob:
            ...
            logger.warning(f"DB saqlandi ⚠️ (faqat JSONBlob + lokal) — {n} kino")
        else:
            logger.warning(f"⚠️ Onlayn saqlanmadi, faqat lokal backup — {n} kino")
...
def db_save(data):
    ...
    with _save_lock:
        ...
        if ok_bin and ok_blob:
            ...
            logger.warning(f"DB saqlandi ⚠️ (sync: faqat JSONBlob + lokal) — {n} kino")
        else:
    ...
    async def _periodic_refresh(context):
        ...
        try:
            ...
            for key, value in fresh.get("btn_texts", {}).items():
                DB.setdefault("btn_texts", {})[key] = value
            for key, value in fresh.get("emoji_ids", {}).items():
                DB.setdefault("emoji_ids", {})[key] = value
                EMOJI_IDS[key] = value
            ...
            if fresh.get("settings"):
                DB.setdefault("settings", {}).update({
                    k: v for k, v in fresh.get("settings", {}).items()
                    if v and not DB.setdefault("settings", {}).get(k)
                })
            after = len(DB.get("movies", {}))
            if after != before:
                logger.info(f"🔄 JSONBin refresh: {before} → {after} kino xotirada")
        except Exception as e:
            logger.error(f"Periodik refresh xato: {e}")

    if app.job_queue:
        app.job_queue.run_repeating(_periodic_refresh, interval=180, first=180)
        logger.info("🔄 Periodik JSONBin refresh yoqildi (har 3 daqiqada)")

    logger.info(f"Bot ishga tushdi! v10 — {len(DB.get('movies', {}))} kino xotirada")
