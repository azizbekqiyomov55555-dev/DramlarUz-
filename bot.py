...
def db_load():
    """ASOSIY: JSONBin → lokal backup.
    JSONBlob FALLBACK SIFATIDA OLIB TASHLANDI — chunki u eskirgan ma'lumot bilan
    JSONBin ustiga yozish xavfini olib kelardi (kinolar yo'qolib qolardi)."""
    global _load_ok
    ...
    # 2-bosqich: JSONBin javob berdi-yu, lekin bo'sh ekan — bu HAQIQATAN bo'sh bin
    ...
    # 3-bosqich: JSONBin umuman javob bermadi VA lokal backup ham yo'q
    # → SAQLASHNI BLOKLAYMIZ (eski ma'lumotlarni o'chirib yubormaslik uchun)
    logger.error("⛔ JSONBin javob bermayapti VA lokal backup yo'q!")
    logger.error("⛔ Saqlash BLOKLANDI — yangi kinolar JSONBin tiklanmaguncha qabul qilinmaydi.")
    _load_ok = False  # MUHIM: saqlash bloklanadi
...
async def db_save_async(data):
    """Async saqlash: lokal backup + JSONBin (asosiy)."""
    if not _load_ok:
    ...
    # 2) JSONBin'ga yozish (asosiy uzoq muddatli saqlash)
    ok_bin = False
    try:
        async with aiohttp.ClientSession() as session:
            ok_bin = await _save_jsonbin_async(session, payload)
    except Exception as e:
        logger.error(f"db_save_async session xato: {e}")

    if ok_bin:
        logger.info(f"DB saqlandi ✓ (JSONBin + backup) — "
                    f"{len(data.get('movies', {}))} kino")
    else:
        logger.warning("⚠️ JSONBin'ga yozilmadi — faqat lokal backup mavjud")
    return ok_bin


def db_save(data):
    """Sinxron saqlash: lokal backup + JSONBin (asosiy)."""
    if not _load_ok:
        logger.error("⛔ DB yuklanmagan — saqlash bloklandi")
        return False
    with _save_lock:
        ...
        if ok_bin:
            logger.info(f"DB saqlandi ✓ (sync: JSONBin + backup) — "
                        f"{len(data.get('movies', {}))} kino")
        else:
            logger.warning("⚠️ Sync: JSONBin'ga yozilmadi — faqat lokal backup")
        return ok_bin
