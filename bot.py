...
"""
import logging, asyncio, json, time, re, os, threading, copy, atexit
...
# ─── KONFIGURATSIYA ────────────────────────────────────────
# Hostingda quyidagilarni ENV qilib kiriting:
# BOT_TOKEN, ADMIN_ID, SUPABASE_URL, SUPABASE_KEY (service_role).
# Eslatma: bot tokenini reset qilsangiz ham baza o'chmaydi, chunki kino ma'lumotlari
# token ichida emas, alohida doimiy bazada saqlanadi.
BOT_TOKEN = os.environ.get("BOT_TOKEN", "").strip()
ADMIN_ID  = int(os.environ.get("ADMIN_ID", "8537782289"))

# ─── Supabase — ASOSIY baza (eng ishonchli) ────────────────
SUPABASE_URL    = os.environ.get("SUPABASE_URL", "").strip().rstrip("/")
SUPABASE_KEY    = (os.environ.get("SUPABASE_KEY") or os.environ.get("SUPABASE_SERVICE_ROLE_KEY") or "").strip()
SUPABASE_TABLE  = os.environ.get("SUPABASE_TABLE", "bot_storage").strip()
SUPABASE_ROW_ID = int(os.environ.get("SUPABASE_ROW_ID", "1"))
SUPABASE_ENABLED = bool(SUPABASE_URL and SUPABASE_KEY)
SUPABASE_REST   = f"{SUPABASE_URL}/rest/v1/{SUPABASE_TABLE}" if SUPABASE_ENABLED else ""
SUPABASE_HEADERS = {
    "apikey": SUPABASE_KEY,
...
# ─── JSONBlob — ZAXIRA (ishonchsiz, ID o'zgarib ketishi mumkin) ──
...
_async_save_lock = None
_save_tasks = set()
_load_ok = False  # Doimiy baza muvaffaqiyatli yuklanganmi?
_loaded_movie_count = 0
_storage_warning_sent = False
...
def _load_from_supabase():
    """Supabase'dan yuklash. Muvaffaqiyatli bo'lsa dict, aks holda None."""
    if not SUPABASE_ENABLED:
        return None
    for attempt in range(3):
        try:
            r = requests.get(
                f"{SUPABASE_REST}?id=eq.{SUPABASE_ROW_ID}&select=data",
            ...
            if r.status_code == 200:
                ...
                if isinstance(rows, list) and rows:
                    ...
                    return _normalize_db(data)
                return None
            logger.error(f"Supabase load status {r.status_code}: {r.text[:200]}")
        except Exception as e:
            logger.error(f"Supabase load #{attempt+1}: {e}")
        if attempt < 2:
            time.sleep(2)
    return None
...
def _has_real_content(data):
    """DB bo'shmi yoki haqiqiy ma'lumotmi."""
    if not isinstance(data, dict):
        return False
    return bool(data.get("movies")) or bool(data.get("channels")) or bool(data.get("card_number"))


def _merge_db_sources(items):
    """Bir nechta bazani birlashtiradi: kinolar hech qachon users-only eski baza bilan yo'qolmaydi."""
    merged = json.loads(json.dumps(DEFAULT_DB))
    source_names = []
    for name, data in items:
        if not isinstance(data, dict):
            continue
        data = _normalize_db(copy.deepcopy(data))
        source_names.append(name)

        merged.setdefault("users", {}).update(data.get("users", {}))
        merged.setdefault("movies", {}).update(data.get("movies", {}))
        merged.setdefault("pending_payments", {}).update(data.get("pending_payments", {}))
        merged.setdefault("btn_texts", {}).update(data.get("btn_texts", {}))
        merged.setdefault("emoji_ids", {}).update(data.get("emoji_ids", {}))

        if data.get("card_number"):
            merged["card_number"] = data.get("card_number")
        ...
        if isinstance(data.get("stats"), dict):
            merged.setdefault("stats", {})["total_views"] = max(
                int(merged.get("stats", {}).get("total_views", 0) or 0),
                int(data.get("stats", {}).get("total_views", 0) or 0),
            )

        def _norm_channel_key(value: str) -> str:
            value = (value or "").strip().split("?")[0].rstrip("/")
        ...
        seen_channels = {
            _norm_channel_key(ch.get("username", ""))
            for ch in merged.get("channels", []) if isinstance(ch, dict)
        }
        for ch in data.get("channels", []):
            if not isinstance(ch, dict):
                continue
            key = _norm_channel_key(ch.get("username", "") or ch.get("url", ""))
            if key and key not in seen_channels:
                merged.setdefault("channels", []).append(ch)
                seen_channels.add(key)

    return _normalize_db(merged), source_names


def db_load():
    """ASOSIY: Supabase + JSONBlob + lokal backupni birlashtirib yuklaydi."""
    global _load_ok, _loaded_movie_count
    sources = [
        ("Supabase", _load_from_supabase),
    ...
    if loaded:
        data, source_names = _merge_db_sources(loaded)
        EMOJI_IDS.clear()
        EMOJI_IDS.update(data.get("emoji_ids", {}))
        _load_ok = True
        _loaded_movie_count = len(data.get("movies", {}))
        _save_local(json.dumps(data, ensure_ascii=False))
        logger.info(f"✅ DB yuklandi ({', '.join(source_names)}): "
    ...
    logger.warning("⚠️ Kino yo'qolmasligi uchun SUPABASE_URL va SUPABASE_KEY ni hosting ENV ga kiriting!")
    _load_ok = True
    _loaded_movie_count = 0
    return json.loads(json.dumps(DEFAULT_DB))


def refresh_db_from_storage():
    """Xotiradagi DB'ni onlayn/lokal bazadagi yangi ma'lumotlar bilan birlashtiradi."""
    global DB, _loaded_movie_count
    fresh = []
    for name, fn in [("Supabase", _load_from_supabase), ("JSONBlob", _load_from_jsonblob), ("Lokal backup", _load_from_local)]:
        data = fn()
        if data is not None:
            fresh.append((name, data))
    if not fresh:
        return False
    fresh.append(("Xotira", DB))
    merged, source_names = _merge_db_sources(fresh)
    DB = merged
    EMOJI_IDS.clear()
    EMOJI_IDS.update(DB.get("emoji_ids", {}))
    _loaded_movie_count = len(DB.get("movies", {}))
    _save_local(json.dumps(DB, ensure_ascii=False))
    logger.info(f"🔄 DB yangilandi ({', '.join(source_names)}): {_loaded_movie_count} kino")
    return True
...
def _save_supabase_sync(data_dict):
    """Supabase'ga upsert (id=1)."""
    if not SUPABASE_ENABLED:
        logger.warning("Supabase ENV kiritilmagan — faqat JSONBlob/lokal backup ishlaydi")
        return False
    body = json.dumps([{"id": SUPABASE_ROW_ID, "data": data_dict}],
                      ensure_ascii=False).encode("utf-8")
    for attempt in range(3):
        try:
            r = requests.post(SUPABASE_REST, headers=SUPABASE_HEADERS,
                              data=body, timeout=30)
            if r.status_code in (200, 201, 204):
                return True
            logger.error(f"Supabase status {r.status_code}: {r.text[:200]}")
        except Exception as e:
            logger.error(f"Supabase save #{attempt+1}: {e}")
        if attempt < 2:
            time.sleep(2)
    return False
...
async def _save_supabase_async(session, data_dict):
    if not SUPABASE_ENABLED:
        return False
    body = json.dumps([{"id": SUPABASE_ROW_ID, "data": data_dict}],
                      ensure_ascii=False).encode("utf-8")
    for attempt in range(3):
        try:
            async with session.post(
                SUPABASE_REST, data=body, headers=SUPABASE_HEADERS,
                timeout=aiohttp.ClientTimeout(total=30)
            ) as resp:
                if resp.status in (200, 201, 204):
                    return True
                txt = await resp.text()
                logger.error(f"Supabase status {resp.status}: {txt[:200]}")
        except Exception as e:
            logger.error(f"Supabase async #{attempt+1}: {e}")
        if attempt < 2:
            await asyncio.sleep(2)
    return False


def _save_is_safe(data):
    """Tasodifan bo'sh DB eski kinolarni ustidan yozib yubormasin."""
    movie_count = len(data.get("movies", {})) if isinstance(data, dict) else 0
    if _loaded_movie_count > 0 and movie_count == 0:
        logger.error("⛔ Saqlash bloklandi: xotirada 0 kino, yuklanganda kino bor edi")
        return False
    return True


async def _get_async_save_lock():
    global _async_save_lock
    if _async_save_lock is None:
        _async_save_lock = asyncio.Lock()
    return _async_save_lock


async def db_save_async(data):
    """Async saqlash: lokal + JSONBlob + Supabase. Bir vaqtda yozishlar navbatga qo'yiladi."""
    global _loaded_movie_count
    if not _load_ok:
        logger.error("⛔ DB yuklanmagan — saqlash bloklandi")
        return False
    if not _save_is_safe(data):
        return False

    lock = await _get_async_save_lock()
    async with lock:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ok_blob, ok_sb = await asyncio.gather(
                    _save_jsonblob_async(session, payload),
                    _save_supabase_async(session, data),
                    return_exceptions=False,
                )
        except Exception as e:
            logger.error(f"db_save_async session xato: {e}")

        if ok_blob and ok_sb:
            logger.info("DB saqlandi ✓ (JSONBlob + Supabase + backup)")
        elif ok_blob or ok_sb:
            which = "JSONBlob" if ok_blob else "Supabase"
        ...
        if ok_blob or ok_sb:
            _loaded_movie_count = len(data.get("movies", {}))
        return ok_blob or ok_sb


def db_save(data):
    """Sinxron saqlash: lokal + JSONBlob + Supabase."""
    global _loaded_movie_count
    if not _load_ok:
        logger.error("⛔ DB yuklanmagan — saqlash bloklandi")
        return False
    ...
    with _save_lock:
        ...
        ok_sb   = _save_supabase_sync(data)
        if ok_blob and ok_sb:
            logger.info("DB saqlandi ✓ (sync: JSONBlob + Supabase)")
        elif ok_blob or ok_sb:
            which = "JSONBlob" if ok_blob else "Supabase"
...
def save():
    """Asosiy saqlash funksiyasi — event loop yopilib qolsa ham task yo'qolmaydi."""
    try:
...
async def flush_save_tasks():
    if _save_tasks:
        await asyncio.gather(*list(_save_tasks), return_exceptions=True)


def flush_save_tasks_sync():
    try:
        save_sync()
    except Exception as e:
        logger.error(f"Chiqishda DB saqlash xatosi: {e}")


atexit.register(flush_save_tasks_sync)
...
def normalize_movie_code(value: str) -> str:
    """Kino kodini bir xil formatga keltiradi: bosh/oxirgi probel, ichki bo'shliqlar olib tashlanadi."""
    return re.sub(r"\s+", "", (value or "").strip()).upper()


async def handle_movie_code(update: Update, context: ContextTypes.DEFAULT_TYPE, raw_text: str, *, reply_if_missing: bool = False) -> bool:
    """Kino kodini 1-martadayoq topish uchun xotira + bazadan tekshiradi."""
    user = update.effective_user
    uid = user.id
    code = normalize_movie_code(raw_text)
    if not code:
        return False

    if code not in DB.get("movies", {}):
        refresh_db_from_storage()
...
async def send_movie_menu(src, context, code):
    code = normalize_movie_code(code)
    ...
    try:
        if poster:
            try:
                ...
                logger.error(f"Poster yuborilmadi, matnli menyu yuboriladi: {poster_error}")
                await sm(context.bot, chat_id, caption, markup)
        else:
            await sm(context.bot, chat_id, caption, markup)
    except Exception as e:
        logger.error(f"send_movie_menu: {e}")
        await sm(context.bot, chat_id, "❌ Kino menyusini yuborishda xato bo'ldi. Qayta urinib ko'ring.")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and args[0].startswith("code_"):
        code = normalize_movie_code(args[0].replace("code_", ""))
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_help"):
        # Agar foydalanuvchi Yordamdan keyin kino kodi yuborsa, xabar admin'ga ketmaydi — kino darhol chiqadi.
        if await handle_movie_code(update, context, text, reply_if_missing=False):
            return
        context.user_data.pop("awaiting_help", None)
    ...
    # ── 12. Kino kodi ──
    await handle_movie_code(update, context, text, reply_if_missing=True)
...
async def admin_state_handler(update, context, text):
    ...
    if state == "delete_movie_code":
        code = normalize_movie_code(text)
    ...
    if state == "add_movie_code":
        context.user_data["new_movie_code"] = normalize_movie_code(text)
